...
사용자가 평소 말투로 기능 변경을 요청하면 AI가 자동으로 workflow를 시작한다.
사용자가 spec 파일을 찾거나 복사할 필요는 없다.

1. 코드·hook·service·runtime 동작이 바뀌는 요청이면 AI가
   `docs/specs/feature-change-template.md`를 바탕으로 작업별 spec 초안을 만든다.
2. AI는 필요한 딥인터뷰 질문을 한 번에 묶어 묻고, 질문이 없어도 되는 단순한
   변경은 최소 spec을 작성해 보여준다.
3. 사용자가 승인하기 전에는 코드를 수정하지 않는다. "진행", "구현해", "고쳐"
   같은 말도 spec 승인으로 간주하지 않고, spec을 먼저 제시한다.
4. 사용자가 명시적으로 승인하면 그때 구현·테스트·PR을 진행한다.

문서-only 요청은 이 spec gate를 거치지 않고 직접 문서를 갱신한다. 사용자가
"spec 작성", "계획", "검토"만 요청한 경우에는 코드 구현 없이 해당 산출물만 만든다.
