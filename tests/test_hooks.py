...
def test_claude_agent_wording_stays_user_without_dispatch_event(hook_env):
...
def test_claude_agent_dispatch_is_assistant_and_echo_is_skipped(hook_env):
    bc.write_json_atomic(bc.REGISTER / "claude-parent.json", {"session_id": "claude-parent"})
    prompt = "Inspect the task and report findings."
    r = run_hook(
        "agent_dispatch.py",
        {"session_id": "claude-parent", "tool_name": "Agent", "tool_input": {"prompt": prompt}},
        hook_env,
    )
    assert r.returncode == 0
    r = run_hook(
        "user_prompt_submit.py",
        {"session_id": "claude-parent", "prompt": prompt},
        hook_env,
    )
    assert r.returncode == 0
    files = list((bc.OUTBOX / "claude-parent").iterdir())
    assert len(files) == 1
...
def test_codex_agent_wording_stays_user_without_dispatch_event(hook_env):
...
def test_codex_session_wording_stays_user_without_dispatch_event(hook_env):
...
def test_codex_agent_dispatch_is_assistant_and_echo_is_skipped(hook_env):
    bc.write_json_atomic(bc.REGISTER / "cx-parent.json", {"session_id": "cx-parent", "kind": "codex"})
    prompt = "Review the requested change."
    r = run_hook(
        "agent_dispatch.py",
        {
            "session_id": "cx-parent",
    ...
    r = run_hook(
        "codex_user_prompt_submit.py",
        {"session_id": "cx-parent", "prompt": prompt},
        hook_env,
    )
    assert r.returncode == 0
    files = list((bc.OUTBOX / "cx-parent").iterdir())
    assert len(files) == 1
    assert json.loads(files[0].read_text()) == {"role": "assistant", "text": prompt}
