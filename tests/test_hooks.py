...
def test_codex_prompt_role_is_not_guessed_from_text(hook_env):
    prompts = (
        "당신은 backnote-po 제품 반영 검수 agent입니다. 산출물을 직접 읽어 판정하세요.",
        "🐮 저장소 /home/breadum/Work/backnote\ncwd: /tmp\ntask: 008 구현을 진행하라",
    )
    for index, prompt in enumerate(prompts):
        sid = f"cx-prompt-{index}"
